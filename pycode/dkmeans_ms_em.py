"""
Compute Multi-Shot K-Means with Expectation Maximization
...
from dkmeans_util import (plot_clustering, simulated_gaussian_cluster,
                          ...
                          save_frames_to_gif)
from dfnc_load_data import window_all_tc, load_exemplar
from scipy.spatial.distance import pdist, squareform, cdist


def dkm_ms_em_remote_gaussian_centroids(n, m, k, mu=0.0, sigsq=1.0):
    """
...
def dkm_ms_em_local_initialize_own_centroids(local_D, k):
...
def dkm_ms_em_local_compute_mean(local_D, local_C, k):
    """
    For MS-EM, compute the local mean, which is broadcast back to the agg
    """
    M = [[] for i in range(k)]
...
def dkm_ms_em_remote_update_centroids(remote_M, w, k):
    """
...
def dkm_ms_em_local_compute_clustering(local_D, remote_w):
    C = []
    Z = []
    for d in local_D:
...
def main(X, k, s=2, ep=0.00001):
    ...
    w = []
    for si in range(s):
        wi = dkm_ms_em_local_initialize_own_centroids(nodes[si], k)
        w += wi
    np.random.shuffle(w)
    w = w[:k]
    b = True
    ...
    while b:
        ...
        Z = []
        for node in nodes:
            Ci, Zi = dkm_ms_em_local_compute_clustering(node, w)
            Cl += [Ci]
            liM = dkm_ms_em_local_compute_mean(node, Ci, k)
            lM = [lM[i] + [liM[i]] for i in range(k)]
            C += Ci
            Z += Zi
        [w, w0] = dkm_ms_em_remote_update_centroids(lM, w, k)
        b, delta = dkm_ms_em_remote_check_stopping(w, w0, ep)
        Del += [delta]
        i += 1
    return {'w': w, 'C': C, 'X': D, 'delta': Del, 'iter': i, 'name': 'ms_em'}
