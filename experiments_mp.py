...
from dkmeans.metrics import average_distance
...
def run_repeated_experiment_mp(R, k, N, metrics=METRIC_NAMES,
                            methods=METHOD_NAMES, **kwargs):
    """
...
def run_repeated_experiment(R, k, N, metrics=METRIC_NAMES,
    ...
    for r in range(R):
        ...
        print("Printing Results k=%d, R=%d, N=%d in process %s" %
              (k, R, N, os.getpid()))
        for method in meas:
...
def dfnc_pipe(k, N, R, s=2, rep=100):
    ...
    meas, res = run_repeated_experiment_mp(rep, k, N,
    ...
    for method in meas:
        ...
        print("Running dFNC second stage k=%d, R=%d, N=%d in process %s" %
              (k, R, N, os.getpid()))
        measR, resR = run_repeated_experiment_mp(1, k, N,
                                              dataset='real_fmri',
                                              verbose=True,
                                              s=s,
...
def main():
    ...
    meas, res = run_repeated_experiment(R, 3, N, theta=theta, verbose=True,
                                        dataset='gaussian')
    ...
    print("Running K experiment dFNC pipeline")
    s = 2
    #processes = []
    dfnc_pipe(5, N, R, s)
    # for k in k_test:
